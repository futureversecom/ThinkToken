...
contract Bridge is IBridge, IBridgeReceiver, Ownable, ReentrancyGuard, ERC165 {
    ...
    function sendMessage(
        ...
        require(
            ...
            "Bridge: msg exceeds max length"
        );
        require(
            msg.value >= _sendMessageFee,
            "Bridge: insufficient message fee"
        );
    ...
    function _verifyMessage(
        ...
        for (uint i; i < _validators.length; ++i) {
            if (proof.r[i] != ommited) {
                ...
                require(
                    _validators[i] ==
                        digest.recover(proof.v[i], proof.r[i], proof.s[i]),
        ...
        require(
            witnessCount >= acceptanceTreshold,
            "Bridge: not enough signatures"
        );
    ...
    function _setValidators(
        ...
        uint reward = Math.min(
            address(this).balance - accumulatedMessageFees,
            maxRewardPayout
        );
