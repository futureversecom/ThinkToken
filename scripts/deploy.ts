...
async function main() {
  ...
  const pegManagerPk = process.env[`${prefix}PEG_MANAGER_PK`];

  const rolesManager = process.env[`${prefix}ROLES_MANAGER`];
  const tokenManager = process.env[`${prefix}TOKEN_MANAGER`];
  const recoveryManager = process.env[`${prefix}TOKEN_RECOVERY_MANAGER`];
  ...
  console.log("Recovery Manager:", recoveryManager);
  ...
  console.log("The recovery manager is:", recoveryManager);
