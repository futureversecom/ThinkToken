...
describe('Bridge', () => {
  async function setup() {
    ...
    const MockBridgeFactory = await ethers.getContractFactory('MockBridgeReceiver');
  ...
  async function callTransitivelyViaReceiveMessage({
    ...
    validatorSetId,
    fee,
  }: { bridge: any, txExecutor: any, owner: any, source: string, destination: string, appMessage: string, validatorSetId: number, fee?: number }) {
    ...
    await bridge.forceActiveValidatorSet([validatorSigner.address], validatorSetId); // set validators
    ...
    const flatSignature = validatorSigner._signingKey().signDigest(bridgeMessageBytes); // sign raw message - without EIP-712 prefix
    ...
    return bridge.connect(txExecutor).receiveMessage(source, destination, appMessage, proof, { value: verificationFee })
  }
  ...
  it('bridge active status', async () => {
    ...
    await expect(bridge.connect(user).setActive(true)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set status to active
    await expect(bridge.connect(owner).setActive(true)).to.emit(bridge, 'BridgeActiveUpdated').withArgs(true);
  ...
  it('bridge pallet address', async () => {
    ...
    await expect(bridge.connect(user).setPalletAddress(newPalletAddress)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set pallet address
    await expect(
      bridge.connect(owner).setPalletAddress(newPalletAddress)
    ).to.emit(bridge, 'PalletAddressUpdated').withArgs(newPalletAddress);
  ...
  it('max message length', async () => {
    ...
    await expect(bridge.connect(user).setMaxMessageLength(newMaxMessageLength)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set max message length
    await expect(
      bridge.connect(owner).setMaxMessageLength(newMaxMessageLength)
    ).to.emit(bridge, 'MaxMessageLengthUpdated').withArgs(newMaxMessageLength);
  ...
  it('validator threshold', async () => {
    ...
    await expect(bridge.connect(user).setThreshold(newThreshold)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set threshold
    await expect(bridge.connect(owner).setThreshold(newThreshold)).to.emit(bridge, 'ThresholdUpdated').withArgs(newThreshold);
  ...
  it('bridge fee', async () => {
    ...
    await expect(bridge.connect(user).setBridgeFee(newFee)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set fee
    await expect(bridge.connect(owner).setBridgeFee(newFee)).to.emit(bridge, 'BridgeFeeUpdated').withArgs(newFee);
  ...
  it('max reward payout', async () => {
    ...
    await expect(bridge.connect(user).setMaxRewardPayout(newMaxRewardPayout)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set max reward payout
    await expect(bridge.connect(owner).setMaxRewardPayout(newMaxRewardPayout)).to.emit(bridge, 'MaxRewardPayoutUpdated').withArgs(newMaxRewardPayout);
  ...
  it('sent event id', async () => {
    ...
    await expect(bridge.connect(user).setSentEventId(5)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set sentEventId
    await expect(bridge.connect(owner).setSentEventId(5)).to.emit(bridge, 'SentEventIdUpdated').withArgs(5);
  ...
  it('proof time to live (TTL)', async () => {
    ...
    await expect(bridge.connect(user).setProofTTL(newProofTTL)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set proof TTL
    await expect(bridge.connect(owner).setProofTTL(newProofTTL)).to.emit(bridge, 'ProofTTLUpdated').withArgs(newProofTTL);
  ...
  it('force historical validator set', async () => {
    ...
    await expect(
      bridge.connect(user).forceHistoricValidatorSet(oldValidators, validatorSetId)
    ...
    const expectedDigest = '0xa9404f191a6a84d0d36e618e9552617f03e20ee60e640f27c07db9245d83e495';
    await expect(
      bridge.connect(owner).forceHistoricValidatorSet(oldValidators, validatorSetId)
    ).to.emit(bridge, 'ForceSetHistoricValidators').withArgs(expectedDigest, validatorSetId);
  });
  ...
  it('force set active validator set', async () => {
    ...
    await expect(
      bridge.connect(user).forceActiveValidatorSet(newValidators, validatorSetId)
    ).to.be.revertedWith('Ownable: caller is not the owner');
    ...
    const expectedDigest = '0xa9404f191a6a84d0d36e618e9552617f03e20ee60e640f27c07db9245d83e495';
    await expect(
      bridge.connect(owner).forceActiveValidatorSet(newValidators, validatorSetId)
    ).to.emit(bridge, 'ForceSetActiveValidators').withArgs(expectedDigest, validatorSetId);
  ...
  it('withdrawAll', async () => {
    ...
    expect(ownerInitBalance.sub(ownerFinalBalance)).to.equal(tx1GasUsed.add(tx2GasUsed));
  });
  ...
  it('sendMessageFee', async () => {
    ...
    await expect(bridge.connect(user).setSendMessageFee(sendMessageFee)).to.be.revertedWith('Ownable: caller is not the owner');

    // verify owner can set send message fee
    await expect(bridge.connect(owner).setSendMessageFee(sendMessageFee)).to.emit(bridge, 'SendMessageFeeUpdated').withArgs(sendMessageFee);
  ...
  it('withdrawMsgFees', async () => {
    ...
    await expect(
      bridge.connect(owner).withdrawMsgFees(destination, msgFee)
    ).to.emit(bridge, 'WithdrawnMessageFees').withArgs(destination, msgFee);
  ...
  it('sendMessage', async () => {
    ...
    await expect(
      bridge.connect(user).sendMessage(destination, msg, { value: utils.parseEther('0.1') })
    ).to.emit(bridge, 'SendMessage').withArgs(0, user.address, destination, msg, utils.parseEther('0.1'));
  ...
  it('receiveMessage - fails', async () => {
    ...
    const proof = { eventId: 1, validatorSetId: 1, v: [], r: [], s: [], validators: [] };
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, [], proof, { value: verificationFee })
  ...
  it('receiveMessage - succeeds', async () => {
    ...
    const appMessage = ethers.utils.defaultAbiCoder.encode(['address[]', 'uint256'], [validators, validatorSetId]);
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proof)
    ).is.revertedWith('Bridge: must supply bridge fee');

    // fails due to inactive bridge
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proof, { value: verificationFee })
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proof, { value: verificationFee })
    ).to.emit(bridge, 'MessageReceived').withArgs(1, source, destination, appMessage);
    ...
    expect(await ethers.provider.getBalance(bridge.address)).to.equal(verificationFee);
  });
  ...
  it('_verifyMessage', async () => {
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, { ...proof, eventId: eventId +1 }, { value: verificationFee })
    ...
    const expandedSignatureUpd = ethers.utils.splitSignature(
      validatorSigner._signingKey().signDigest(
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proofUpd, { value: verificationFee })
  ...
  it('_verifyMessage - no consensus', async () => {
    ...
    await bridge.connect(owner).setActive(true); // activate bridge
    await bridge.forceActiveValidatorSet(       // set single validator
      [validatorSigner.address, validatorSigner.address, validatorSigner.address, validatorSigner.address, validatorSigner.address],
      validatorSetId,
    );
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proofInvalidValidatorSet, { value: verificationFee })
    ...
    const proofUnexpectedValidators = {
      ...
      r: [expandedSignature.r],
      s: [expandedSignature.s],
      validators: [ // more validators than those set from `forceActiveValidatorSet`
        validatorSigner.address,
        validatorSigner.address,
        validatorSigner.address,
        validatorSigner.address,
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proofUnexpectedValidators, { value: verificationFee })
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proofIncorrectValidator, { value: verificationFee })
    ).to.be.revertedWith('Bridge: unexpected validator digest');

    const omitted = utils.formatBytes32String('');
    const proofIncompleteSignatures = { // TODO `not enough signatures`
      eventId,
      validatorSetId,
      ...
      validators: [validatorSigner.address, validatorSigner.address, validatorSigner.address, validatorSigner.address, validatorSigner.address], // more validators than those set from `forceActiveValidatorSet`
    };
    ...
    await expect(
      bridge.connect(user).receiveMessage(source, destination, appMessage, proofIncompleteSignatures, { value: verificationFee })
  ...
  it('onMessageReceived', async () => {
    ...
    await expect(
      callTransitivelyViaReceiveMessage({
        bridge, txExecutor: user, owner, source: user.address, destination, appMessage, validatorSetId
      })
  ...
  it('_setValidators', async () => {
    ...
    const msg1 = ethers.utils.defaultAbiCoder.encode(['address[]', 'uint256'], [[], validatorSetId]);
    await expect(
      callTransitivelyViaReceiveMessage({
        bridge, txExecutor: user, owner, source, destination, appMessage: msg1, validatorSetId
      })
    ).to.revertedWith('Bridge: empty validator set');

    // fails if validator id is the same
    const msg2 = ethers.utils.defaultAbiCoder.encode(['address[]', 'uint256'], [validators, validatorSetId]);
    await expect(
      callTransitivelyViaReceiveMessage({
        bridge, txExecutor: user, owner, source, destination, appMessage: msg2, validatorSetId
      })
    ).to.revertedWith('Bridge: validator set id replayed');

    const msgValidatorSetId = 2;
    const msg3 = ethers.utils.defaultAbiCoder.encode(['address[]', 'uint256'], [validators, msgValidatorSetId]); // validatorSetId = 2
    const expectedValidatorSetDigest = '0xa9404f191a6a84d0d36e618e9552617f03e20ee60e640f27c07db9245d83e495'; // keccak256(validators)
    const userBalanceBeforeTx = await user.getBalance();
    const tx = await callTransitivelyViaReceiveMessage({
      bridge, txExecutor: user, owner, source, destination, appMessage: msg3, validatorSetId, fee: 0
    });
    ...
    expect(userNewBalance.sub(userBalanceBeforeTx).toString()).to.equal(balanceChangeExpected.toString());
    ...
    expect(await bridge.validatorSetDigests(msgValidatorSetId)).equals(expectedValidatorSetDigest);
  });
})
